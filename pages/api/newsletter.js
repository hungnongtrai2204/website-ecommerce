...
router.post(async (req, res) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ message: "Vui lòng thêm một địa chỉ email." });
    }
    const { url, data, headers } = mailchiphandler(email);
    await axios.post(url, data, { headers });
    return res.status(200).json({
      message: "Bạn đã được thêm vào bản tin của chúng tôi thành công.",
    });
  } catch (error) {
...
function mailchiphandler(email) {
